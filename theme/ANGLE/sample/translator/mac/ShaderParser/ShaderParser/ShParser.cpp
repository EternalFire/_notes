...
namespace _Sh_ {
	...
	{
		if (vsUniformMap.find(shaderName) == vsUniformMap.end())
		{
			UniformArray array;
			vsUniformMap[shaderName] = array;
		}
		...
		vsUniformMap[shaderName].push_back(st);
	}

	void addFsUniform(const std::string& shaderName, VarType type, const std::string& name)
	{
		if (fsUniformMap.find(shaderName) == fsUniformMap.end())
		{
			UniformArray array;
			fsUniformMap[shaderName] = array;
		}
		...
		fsUniformMap[shaderName].push_back(st);
	}
}
...
static void PrintVariable(const std::string& prefix, size_t index, const sh::ShaderVariable& var, char* filename);
static void PrintActiveVariables(ShHandle compiler, char* filename, sh::GLenum shaderType);
...
int run(int argc, char* argv[])
{
	...
	for (; (argc >= 1) && (failCode == ESuccess); argc--, argv++)
	{
		...
		else
		{
			...
			ShHandle compiler = 0;
			sh::GLenum shaderType = FindShaderType(filename);
...
sh::GLenum FindShaderType(const char* fileName)
{
	...
	if (ext)
	{
		if (strncmp(ext, ".frag", 5) == 0 || strncmp(ext, ".fs", 3) == 0)
			return GL_FRAGMENT_SHADER;
		if (strncmp(ext, ".vert", 5) == 0 || strncmp(ext, ".vs", 3) == 0)
...
void PrintVariable(const std::string & prefix, size_t index, const sh::ShaderVariable & var, char* filename, sh::GLenum shaderType)
{
	std::string typeName;
	switch (var.type)
	{
...
static void PrintActiveVariables(ShHandle compiler, char* filename, sh::GLenum shaderType)
{
	...
	for (size_t varCategory = 0; varCategory < 5; ++varCategory)
	{
		...
		for (size_t i = 0; i < numVars; ++i)
		{
			...
			PrintVariable(varCategoryName, i, *var, filename, shaderType);
		}
		printf("\n");
	}
}
